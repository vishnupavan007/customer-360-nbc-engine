...
with col1:
    total = session.sql("SELECT COUNT(*) AS CNT FROM CUSTOMER_360.CURATED.CUSTOMER_360_UNIFIED").to_pandas()
    st.metric("Total Customers", f"{total['CNT'].iloc[0]:,}")

with col2:
    active = session.sql("SELECT COUNT(*) AS CNT FROM CUSTOMER_360.CURATED.CUSTOMER_360_UNIFIED WHERE IS_ACTIVE = TRUE").to_pandas()
    st.metric("Active Customers", f"{active['CNT'].iloc[0]:,}")

with col3:
    high_risk = session.sql("SELECT COUNT(*) AS CNT FROM CUSTOMER_360.AI.DT_CHURN_RISK WHERE CHURN_RISK_SCORE >= 0.7").to_pandas()
    st.metric("High Churn Risk", f"{high_risk['CNT'].iloc[0]:,}")

with col4:
    pending_actions = session.sql("SELECT COUNT(*) AS CNT FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION WHERE PRIORITY = 'High'").to_pandas()
