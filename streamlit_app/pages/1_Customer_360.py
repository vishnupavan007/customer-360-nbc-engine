...
if search_term:
    if search_term.strip().isdigit():
        ...
        where_clause = f"WHERE LOWER(c.FULL_NAME) LIKE '%{safe}%'"

    customers = session.sql(f"""
        SELECT c.CUSTOMER_ID, c.FULL_NAME, c.EMAIL, c.PHONE, c.CUSTOMER_SEGMENT,
               ...
               c.TOTAL_OUTSTANDING_BALANCE, c.MAX_DAYS_PAST_DUE, c.COMPLAINT_COUNT,
               c.TOTAL_CALLS, c.COMPOSITE_RISK_LEVEL,
        ...
        for _, row in customers.iterrows():
            with st.expander(f"{row['FULL_NAME']} (ID: {row['CUSTOMER_ID']}) — {row['CUSTOMER_SEGMENT']}", expanded=len(customers) == 1):
                ...
                with tab3:
                    ...
                    loans = session.sql(f"""
                        SELECT LOAN_TYPE, LOAN_STATUS, PRINCIPAL_AMOUNT,
                               OUTSTANDING_BALANCE, DAYS_PAST_DUE, DELINQUENCY_CATEGORY
                ...
                with tab4:
                    timeline = session.sql(f"""
                        SELECT EVENT_DATE, EVENT_CATEGORY, EVENT_DESCRIPTION, EVENT_STATUS
                ...
                churn_nba = session.sql(f"""
                    SELECT cr.CHURN_RISK_SCORE, cr.RETENTION_URGENCY, cr.MODEL_CONFIDENCE,
