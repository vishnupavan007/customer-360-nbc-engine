...
results.append({"Test": "Python Version", "Status": "", "Detail": ""})
try:
    import sys
    results[-1]["Status"] = sys.version.split()[0]
except Exception:
...
with st.expander("Widget Tests (collapsed to reduce visual noise)", expanded=False):
    ...
    # Input widgets (every type used in the app)
    ...
    # Sidebar widgets (used by AI Advisor, Operations)
    ...
    # Styled dataframe (used by Churn Risk, Operations)
    test("styled dataframe", lambda: st.dataframe(df_test.style.highlight_max(axis=0)))

    # st.success / st.error / st.warning / st.info (used across pages)
    ...
    # Critical: st.experimental_rerun (used by AI Advisor + Operations)
    ...
    # st.stop (used by error handling)
...
if session:
    ...
    test("Home: Sentiment Distribution", lambda: run_query(
        ...
        "FROM CUSTOMER_360.AI.DT_TRANSCRIPT_SENTIMENT GROUP BY SENTIMENT_LABEL"))

    test("Home: Top Action Items (15 rows)", lambda: run_query(
        "SELECT FULL_NAME, ACTION_TYPE, PRIORITY, RECOMMENDED_CHANNEL "
        "FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE PRIORITY='High' AND ACTION_TYPE IS NOT NULL "
        "ORDER BY CHURN_RISK_SCORE DESC LIMIT 15"))
    ...
    test("Cust360: Search ILIKE 'smith'", lambda: run_query(
        ...
        "WHERE FULL_NAME ILIKE '%smith%' LIMIT 5"))
    ...
    test("Cust360: Claims for customer", lambda: run_query(
        "SELECT CLAIM_ID, CLAIM_TYPE, CLAIM_STATUS, CLAIM_AMOUNT "
        "FROM CUSTOMER_360.CLEAN.DT_CLAIMS LIMIT 3"))

    test("Cust360: Loans for customer", lambda: run_query(
        "SELECT LOAN_ID, LOAN_TYPE, LOAN_STATUS, OUTSTANDING_BALANCE "
        "FROM CUSTOMER_360.CLEAN.DT_LOANS LIMIT 3"))

    test("Cust360: Timeline for customer", lambda: run_query(
        "SELECT EVENT_DATE, EVENT_CATEGORY, EVENT_DESCRIPTION "
        "FROM CUSTOMER_360.CURATED.CUSTOMER_INTERACTION_TIMELINE LIMIT 3"))

    test("Cust360: Segment overview", lambda: run_query(
        "SELECT CUSTOMER_SEGMENT, COUNT(*) AS CNT "
        "FROM CUSTOMER_360.CURATED.CUSTOMER_360_UNIFIED GROUP BY CUSTOMER_SEGMENT"))

    # --- Churn Risk page ---
    test("Churn: Segment filter values", lambda: run_query(
        "SELECT DISTINCT CUSTOMER_SEGMENT FROM CUSTOMER_360.AI.DT_CHURN_RISK "
        "WHERE CHURN_RISK_SCORE IS NOT NULL ORDER BY CUSTOMER_SEGMENT"))

    test("Churn: Filtered by segment+slider", lambda: run_query(
        "SELECT FULL_NAME, CHURN_RISK_SCORE, RETENTION_URGENCY "
        "FROM CUSTOMER_360.AI.DT_CHURN_RISK "
        "WHERE CUSTOMER_SEGMENT='Premium' AND CHURN_RISK_SCORE >= 0.0 "
        "ORDER BY CHURN_RISK_SCORE DESC LIMIT 5"))

    test("Churn: Risk distribution buckets", lambda: run_query(
        "SELECT CASE WHEN CHURN_RISK_SCORE>=0.8 THEN 'Critical' "
        ...
        "COUNT(*) AS CNT FROM CUSTOMER_360.AI.DT_CHURN_RISK "
        "WHERE CHURN_RISK_SCORE IS NOT NULL GROUP BY 1"))
    ...
    test("Sentiment: By call reason", lambda: run_query(
        ...
        "GROUP BY CALL_REASON, SENTIMENT_LABEL LIMIT 10"))

    test("Sentiment: Negative calls detail", lambda: run_query(
        "SELECT TRANSCRIPT_ID, CUSTOMER_ID, CALL_REASON, SENTIMENT_SCORE "
        "FROM CUSTOMER_360.AI.DT_TRANSCRIPT_SENTIMENT "
        "WHERE SENTIMENT_LABEL='Negative' LIMIT 5"))

    test("Sentiment: Transcript text exists", lambda: run_query(
        "SELECT TRANSCRIPT_ID, LEFT(TRANSCRIPT_TEXT, 100) AS PREVIEW "
        "FROM CUSTOMER_360.AI.DT_TRANSCRIPT_SENTIMENT LIMIT 1"))

    # --- Next Best Action page ---
    test("NBA: Priority filter values", lambda: run_query(
        "SELECT DISTINCT PRIORITY FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE PRIORITY IS NOT NULL"))

    test("NBA: Action type filter values", lambda: run_query(
        "SELECT DISTINCT ACTION_TYPE FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE ACTION_TYPE IS NOT NULL"))

    test("NBA: Filtered action queue", lambda: run_query(
        "SELECT FULL_NAME, ACTION_TYPE, PRIORITY, RECOMMENDED_CHANNEL, RATIONALE "
        "FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE PRIORITY='High' AND ACTION_TYPE='Retention_Offer' LIMIT 5"))

    test("NBA: Actions by channel grouping", lambda: run_query(
        "SELECT RECOMMENDED_CHANNEL, COUNT(*) AS CNT "
        "FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE ACTION_TYPE IS NOT NULL GROUP BY RECOMMENDED_CHANNEL"))

    # --- AI Advisor page (all SQL fallback queries) ---
    test("Advisor: Top churn risk query", lambda: run_query(
    ...
    test("Advisor: Churn risk distribution", lambda: run_query(
        "SELECT CASE WHEN CHURN_RISK_SCORE>=0.8 THEN 'Critical' ELSE 'Other' END AS B, "
        "COUNT(*) AS CNT FROM CUSTOMER_360.AI.DT_CHURN_RISK "
        "WHERE CHURN_RISK_SCORE IS NOT NULL GROUP BY 1"))

    test("Advisor: Complaint details join", lambda: run_query(
    ...
    test("Advisor: NBA for negative sentiment", lambda: run_query(
        ...
        "FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION "
        "WHERE LATEST_SENTIMENT_LABEL='Negative' AND ACTION_TYPE IS NOT NULL "
    ...
    test("Advisor: Premium high-risk count", lambda: run_query(
        "SELECT COUNT(*) AS N FROM CUSTOMER_360.AI.DT_CHURN_RISK "
        "WHERE CHURN_RISK_SCORE>=0.7 AND CUSTOMER_SEGMENT='Premium'"))

    # --- Operations page ---
    test("Ops: SHOW DYNAMIC TABLES (via collect)", lambda: session.sql(
        "SHOW DYNAMIC TABLES IN DATABASE CUSTOMER_360").collect())
    ...
    # --- AI Functions ---
    test("Cortex: SENTIMENT function", lambda: run_query(
        "SELECT SNOWFLAKE.CORTEX.SENTIMENT('This is great') AS S"))

    test("Cortex: COMPLETE (llama3.1-8b)", lambda: run_query(
        "SELECT SNOWFLAKE.CORTEX.COMPLETE('llama3.1-8b','Reply: ok') AS R"))

    # --- Data Quality ---
    test("DQ: Churn scores in [0,1]", lambda: (
        run_query("SELECT COUNT(*) AS N FROM CUSTOMER_360.AI.DT_CHURN_RISK "
                  "WHERE CHURN_RISK_SCORE < 0 OR CHURN_RISK_SCORE > 1"),
        "0 violations expected"))

    test("DQ: Sentiment scores in [-1,1]", lambda: (
        run_query("SELECT COUNT(*) AS N FROM CUSTOMER_360.AI.DT_TRANSCRIPT_SENTIMENT "
                  "WHERE SENTIMENT_SCORE < -1 OR SENTIMENT_SCORE > 1"),
        "0 violations expected"))

    test("DQ: Churn completeness >= 90%", lambda: run_query(
        "SELECT ROUND(COUNT(CHURN_RISK_SCORE)*100.0/NULLIF(COUNT(*),0),1) AS PCT "
        "FROM CUSTOMER_360.AI.DT_CHURN_RISK"))

    test("DQ: NBA completeness >= 90%", lambda: run_query(
        "SELECT ROUND(COUNT(ACTION_TYPE)*100.0/NULLIF(COUNT(*),0),1) AS PCT "
        "FROM CUSTOMER_360.AI.DT_NEXT_BEST_ACTION"))
...
total = len(rdf) - 2  # exclude version rows
...
categories = {
    "Streamlit APIs": len([r for r in results if r["Test"].startswith("st.") or r["Test"].startswith("hasattr") or r["Test"].startswith("styled")]),
    "Sidebar Widgets": len([r for r in results if "sidebar" in r["Test"].lower()]),
    "Data Tables": len([r for r in results if r["Test"].startswith("Table:")]),
    "Home Page Queries": len([r for r in results if r["Test"].startswith("Home:")]),
    "Customer 360 Queries": len([r for r in results if r["Test"].startswith("Cust360:")]),
    "Churn Risk Queries": len([r for r in results if r["Test"].startswith("Churn:")]),
    "Sentiment Queries": len([r for r in results if r["Test"].startswith("Sentiment:")]),
    "NBA Queries": len([r for r in results if r["Test"].startswith("NBA:")]),
    "AI Advisor Queries": len([r for r in results if r["Test"].startswith("Advisor:")]),
    "Operations Queries": len([r for r in results if r["Test"].startswith("Ops:")]),
    "AI Functions": len([r for r in results if r["Test"].startswith("Cortex:")]),
    "Data Quality": len([r for r in results if r["Test"].startswith("DQ:")]),
}
cdf = pd.DataFrame([{"Category": k, "Tests": v} for k, v in categories.items() if v > 0])
